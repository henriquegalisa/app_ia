<template>
  <Dialog v-model="componentData.showUploadDialog">
    <template v-slot:activator="{ show, close }">
      <v-list-item @click.stop="show" title="Regression Linear"> </v-list-item>
    </template>
    <v-card>
      <v-card-title>Regression Linear</v-card-title>
      <v-card-text>
        <v-text-field v-model="componentData.column" label="Column" />
        <v-text-field v-model="componentData.target" label="Target" />
        <v-btn @click.stop="uploadFile"
          >Upload to Api - Regression Linear</v-btn
        >
      </v-card-text>
    </v-card>
  </Dialog>
  <Dialog v-model="componentData.showResultDialog">
    <v-card>
      <v-card-title>Regression Linear - Result</v-card-title>
...
async function uploadFile(show: Function, close: Function) {
  try {
    ...
    const endpoint = `regression/linear?column=${componentData.column}&target=${componentData.target}`;
